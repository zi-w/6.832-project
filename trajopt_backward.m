function [ot, ou, oh, process] = trajopt_backward(plant, N, Tmin, Tmax, xDim, uDim, jcost, startpos, endpos)
...
for epoch = 1:Kp
    ...
    for j = 1:Kc
        ...
        for t = 1:N-1
            x1 = ot(:,t+1); u1 = ou(:,t+1);
            [f,df] = plant.dynamics(0,x1,u1);
            F{t} = f;
            A{t} = full(df(:,2:1+xDim));
            B{t} = full(df(:,2+xDim:end));
            eta(:,t) = x1 - ot(:,t) - oh(t)*f;
        end
        ...
        for i = 1:Kt
            ...
            for t = 1:N-1
                x1 = ot(:,t+1); u1 = ou(:,t+1);
                etahat(:,t) = nt(:,t+1) - nt(:,t) - F{t}*(h(t)) - A{t}*(nt(:,t+1)-x1)*oh(t) - B{t}*(u(:,t+1)-u1)*oh(t);
            end
            h >= Tmin/(N-1);
            h <= Tmax/(N-1);
